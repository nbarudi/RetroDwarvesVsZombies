...
public class Skeleton extends ItemModule {
	...
	public void onInteracted(Action action, ItemStack item, Player player, PlayerInteractEvent event) {
		...
		if(!item.getItemMeta().getDisplayName().equals(RDvZ.is.becomeSkeleton.getItemMeta().getDisplayName()))
			return;
	...
	public void runAbility(Player player) {
		...
		pd.mrole = MonsterClass.SKELETON;
		pd.setDwarf(false);
		...
		player.sendMessage("§aYou have become a Skeleton!");
